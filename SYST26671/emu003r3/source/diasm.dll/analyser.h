...
int pop();
int primary();
void generate(char tkn, int val);
...
/* emu8086-edit keep last error message */
char sLastAnalyser_err[80];
...
/* emu8086-edit - I made this function from original main() */
int eval(char *line)
{
	// line[ 1024 ]
	char *cp;
...
#define TOS (Valstk[Valsp-1])	/* top of stack macro */
	...
	for ( opsp=Opbase; opsp<Opsp; ++opsp )
	{
		...
		if ( op == T_CONST )
			 ...
			 */
			// emu8086-edit // push( &Symbols[ Opstk[ opsp ].o_value ] );
			push( (int)&Symbols[ Opstk[ opsp ].o_value ] );
		...
		else if ( op == T_IOR )
		{
			...
			// emu8086-edit err( "bad operator in stack: 0x%x (%c)", op, op );
			err( "bad operator in stack!");
	}
...
int op_prim(int precedence, int lvalue)
// precedence;	/* precedence of current <operator> */
// lvalue;	/* type of current <primary>: -1 => lvalue */
		/*                             0 => no <primary> (error) */
		/*                             1 => rvalue */
{
	/*
...
//emu8086-edit	int i;
	...
	else
	{
		 ...
		 */
		// emu8086-edit err( "bad syntax: %s", Lineptr );
		 err( "bad syntax" );
		++Lineptr;
	}
...
*                       MISCELLANEOUS                        *
**************************************************************/
// emu8086-edit void err( char *a, ... )
void err( char *s )
{
	if ( ! Error )
	{
		//emu8086-edit printf( a );
		//emu8086-edit putchar( '\n' );
		strcpy(sLastAnalyser_err,s);
		Error = 1;
	}
}
